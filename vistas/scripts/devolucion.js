...
function init(){
	...
	$.post("../ajax/devolucion.php?op=selectCliente", function(r){
	            $("#idcliente").html(r);
...
function listarArticulos()
{
	...
	tabla=$('#tblarticulos').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/devolucion.php?op=listarArticulos',
...
function listarArticulosUno(idartubi,idcliente)
{
        ...
        .dataTable(
            {
                ...
                "ajax":{
                    url: '../ajax/devolucion.php?op=listarArticulosUno',
                    data: {idartubi:idartubi,idcliente:idcliente},
					type: "get",
                    dataType:"json",
                    error: function(e) {
                        console.log(e.responseText);
...
{
	$.post("../ajax/devolucion.php?op=mostrar",{iddevolucion : iddevolucion}, function(data, status)
	{
 	...
 	$.post("../ajax/devolucion.php?op=listarDetalle&id="+iddevolucion,function(r){
	        $("#detalles").html(r);
	});	
}
...
function anular(iddevolucion)
{
	bootbox.confirm("¿Está seguro de anular la devolución?", function(result){
		if(result)
        {
        	$.post("../ajax/devolucion.php?op=anular", {iddevolucion : iddevolucion}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
{	
	...
	if (tamañoCant !== 0){  
		listarArticulosUno(idartubi,idcliente);
	}else{
		listarArticulos();
	}
