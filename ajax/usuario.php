    ...
    switch($_GET["op"])
    {
        case 'guardaryeditar':
            ...
            if (empty($idusuario)){
				...
				$rspta = $usuario->verificarid($idusuario);

				$fetch = $rspta->fetch_object();

				if(isset($fetch))
				{
					if ($clave == $fetch->clave)
					{
						$rspta=$usuario->editar($idusuario,$nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$cargo,$imagen,$_POST['permiso']);
						echo $rspta ? "Usuario actualizado" : "Usuario no se pudo actualizar";
					}else{
						$clavehash = hash("SHA256",$clave);
						$rspta=$usuario->editar2($idusuario,$nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$cargo,$clavehash,$imagen,$_POST['permiso']);
						echo $rspta ? "Usuario actualizado" : "Usuario no se pudo actualizar";
					}
				}
